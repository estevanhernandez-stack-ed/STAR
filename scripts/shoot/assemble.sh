...
# 7. sweep — source 8.6s..156.6s (148s) ramped to 20s, counter burned, vo-07 from 0.5s
F=$(python -c "print(148/20)")
ffmpeg -v error -y -ss 8.6 -t 148 -i "$SWEEP" -i "$VO/vo-07-sweep.mp3" \
  -filter_complex "[0:v]setpts=PTS/${F},fps=25,drawtext=fontfile='${FONT}':text='31 SCENES · ONE REQUEST   REAL TIME  %{eif\:floor(t*${F}/60)\:d}\:%{eif\:mod(floor(t*${F})\,60)\:d\:2}':fontcolor=#e9dcc0:fontsize=34:box=1:boxcolor=#1b1f1c@0.85:boxborderw=14:x=w-tw-48:y=48,setsar=1[v];[1:a]adelay=500|500,apad=whole_dur=20[a]" \
  -map "[v]" -map "[a]" -t 20 $VENC $AENC "$SEG/07.mp4"

# 8. numbers — reel 44.3..49.3, 5s, vo-08 from 0.3s
ffmpeg -v error -y -ss 44.3 -t 5 -i "$REEL" -i "$VO/vo-08-numbers.mp3" \
  -filter_complex "[0:v]setsar=1[v];[1:a]adelay=300|300,apad=whole_dur=5[a]" -map "[v]" -map "[a]" -t 5 $VENC $AENC "$SEG/08.mp4"
...
# 10. the catch — 40s: reel 62.3..72.3 (Casbah cluster, 10s) + 13s hold, then check 168..181 (inline marks, 13s) + 4s hold
#     vo-10a from 0.5s (22.1s), vo-10b from 24.5s (13.5s)
