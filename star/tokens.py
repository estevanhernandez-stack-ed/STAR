...
async def list_for(uid: str, store) -> list[McpToken]:
    """Every token this account has issued, newest first. Metadata only."""
    documents = await asyncio.to_thread(store.list_for_uid, uid)
    return [to_metadata(document) for document in documents]
...
async def resolve(
    ...
    return TokenIdentity(uid=uid, token_id=token_id)
