...
INSTRUCTIONS = (
    ...
    # NAMED, NEVER COUNTED. This said "There are six tools" while the door
    # served fourteen, and a test pinned the literal "six tools" — so the
    # sentence could not be corrected without the test failing, which is what
    # made the staleness durable. A count in prose is a second source of truth
    # that only ever drifts one way. README.md already records the same lesson
    # in the same words: name them rather than counting them.
...
TOOLS: tuple[dict, ...] = (
    {
        ...
        "description": (
            ...
            "finish, and `note` is usually that room's account of why, in the "
            "words the writer was given at the time. A failed room still cost "
            "the searches it spent before it stopped, which is why it stays in "
            "the list rather than disappearing: `search_count` says what it "
            "cost and `note` says what happened. Rooms that finished carry an "
            "empty `note` and need none.\n\n"
            "**An empty `note` on a failed room is not a promise that nothing "
            "went wrong.** The field was added after this department had "
            "already filed rooms, so a room that failed before it existed has "
            "none, and a run the process lost mid-build never reached the "
            "branch that writes one. Call `get_room` on any failed room: it "
            "explains the status either way, which `note` alone cannot "
            "guarantee to do.\n\n"
    ...
    },
    {
        ...
        "description": (
            ...
            "**`source_count` is larger than the citations in the drawers, "
            "usually about twice.** It counts every distinct page a search "
            "actually returned during the build — what the department SAW. A "
            "citation is a page a researcher then chose to stand a finding on. "
            "Most of what a search returns is never cited, so the two numbers "
            "differ by design and neither is the other's total. Nothing here "
            "is ever asserted: both come from a ledger of what the searches "
            "returned.\n\n"
