...
Those five rows read `shipped`, not `clean`, and the distinction is load-bearing:
the code exists and is verified, and the wave's close-out — re-capturing the
touched surfaces and re-reviewing them against the measuring stick — has not run
yet. 16 findings remain `open`.
