...
**Close-out ran 2026-08-11, pre-merge. Three of five closed; two did not.**
`F-004`, `F-005` and `F-013` read `clean`. `F-001` and `F-003` went back to
`open`, and the tally is now 3 clean, 18 open, derived from the rows above.
...
`F-001` and `F-003` also missed the brief's own bar. Both are timing-dependent
and the brief required each to be verified twice; two real builds were spent,
one on each. `06-progress-running` was not re-captured — that is a deliberate
cost call, since re-shooting it needs another build, and it means this row's
evidence cell still points at a picture of the defect.
