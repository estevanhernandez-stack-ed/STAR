...
above at that point: **5 clean, 16 open.**
...
**Wave 3 — without sight or a mouse.** F-012, F-006, F-010, F-009, F-021: five
findings invisible in every capture and undetectable by axe or Lighthouse,
because live-region and focus absence are not machine-checkable. That is why
they survived a 67-finding audit and two waves. Built on
`glow/wave-3-without-sight-or-a-mouse`, brief at `docs/glow-wave-3-brief.md`.

**All five closed 2026-08-11, six commits.** Tally derived from the rows above:
**10 clean, 14 open** — of which three of the open rows are `F-022` to `F-024`,
filed by these builds rather than by the audit.

Five fix directions in the rows below were wrong, and as with wave 1 the rows
keep their original text — where they differ, the code is right:

- **F-006's "reuse `scrollBehavior()` and lift it into a shared module" is not
  buildable**, and it contradicts the same row's instruction to let focus do the
  scrolling. `focus()` takes only `preventScroll`; there is no `behavior`
  option, so a focus-driven scroll cannot consume that helper. Nothing in `web/`
  declares `scroll-behavior` at all, which makes the native scroll instant and
  reduced motion honoured without a line of code.
  `tests/js/test_check_result_focus.mjs` asserts that absence across all seven
  stylesheets, so the day someone adds `scroll-behavior: smooth` the suite says
  why it cannot be there.
- **F-006's "named heading" could not be built as written and the obvious
  substitute is broken.** The result body has no heading and its first children
  are two conditional notes, so focusing the meter would step over the cover
  note `scriptcheck.js:453-456` orders first on the record. It became a `region`
  named by the meter line — the name says what landed and every value in it
  comes from the payload. The first attempt named it with `aria-labelledby` at
  the meter's id and **shipped an unnamed region**: Chromium computes no name
  when the target is a descendant of the region, verified both ways in the live
  accessibility tree, while the identical attribute pointing outside names it
  correctly. It passed all 18 source assertions written for it. The fix is
  `aria-label`, and the test now guards the trap as a prohibition, because a
  source assertion cannot see an accessibility tree.
- **F-006's root cause is broader than the mount, and both exits leaked it.**
  `els.run.disabled = true` drops focus to `<body>` for the whole duration of
  the request. The success exit now moves focus into the result; the failure
  exit returns it to the button, after re-enabling, since `focus()` on a
  disabled element is a no-op. The row named the root cause but not the failure
  path.

- **F-010's headline is wrong about its own scope.** "Every failure message in
  the app" misses the two in `account.js` — a failed Google link and a failed
  token issue, both failure-only, both on the surface holding the credential.
  Filed as `F-022` rather than folded in, because they cannot take F-010's fix.
  Its "a class change fires nothing" argument is also over-stated: `alert` is
  specified to announce on becoming visible **or** on content change, and
  implementations differ on the first. The fix is unchanged; the reason is that
  content change is the half everything supports. And the row does not say which
  role `.consent-status` should take — it takes `status`, because WORKING and
  ATTACHING are progress, and progress that interrupts is the defect F-012
  refused to ship on the search meter.
- **F-009's "3.87:1, unrecorded anywhere" is already recorded**, at
  `shell.css:319` as 3.86:1, written when wave 1 shipped the armed control. The
  code cites that line rather than restating the figure — one measurement at two
  precisions is how a palette drifts. The genuinely unrecorded pair is the dot on
  `--cabinet`, 3.28:1, which is now written down. The row's "append the status
  word" is also the wrong shape; see the close-out below.
- **F-021's accessible name is not what the role fixes.** Chromium already
  exposed the label on the bare div, measured before the change as
  `generic "The scene, with each checked line marked"`. The tab stop is the
  entire fix; the role makes the existing name conformant and earns a landmark.
  The row's browser facts were right as corrected in the brief: Safari only.

**The close-out found a defect this wave introduced**, in the half of `F-009` no
test could see. Appending the status to `.rail-room-meta` as the row asked
produced `Autumn 1978 · Aug 10, 2026 · Stopped`, which wraps to a second line in
a 300px rail: the meta grows from 14px to 27px and the row stands taller than
every neighbour — at some era lengths and not others, so it broke the rail's
rhythm on whichever rooms happened to have a long era. Nothing clipped, so
nothing failed.

A flagged row now **replaces** the era rather than appending to it, which is what
the running row already does and for the same two reasons: the era is the weakest
of the three facts, since a run that raised may never have written a
`story_profile` to take it from, and two segments cannot wrap where an ordinary
row's two do not. `tests/js/test_flagged_room.mjs` asserts the segment count
rather than the string, because the count is the property that holds.

Three findings were declined as drive-bys during these builds and filed as
`F-022`, `F-023` and `F-024` instead.

**Wave 4 — the evidence itself.** `F-025`, closed 2026-08-11 in one commit,
because the module, both wirings and both comment corrections are one behaviour
change carrying one test.

The format check the brief insisted on first paid for itself immediately. Both
renderers described an HTML payload with entities; across all 104 stored
excerpts, **5 carry `<strong>`, none carry an entity, and the large majority are
markdown**. Both comments now say what arrives.

**It also corrected the brief.** That said to drop structural lines and join what
was left. Joining manufactures a passage the page never contained, and
`.cite-excerpt` is a `<blockquote>` — so a structural line now ENDS a run
instead, and the quotation is always one contiguous passage.

**The close-out found two defects the wave introduced.** `scriptcheck.js`'s
corrected comment cited 2 `<strong>` excerpts against the 104-excerpt sentence,
when 2 was the single-room figure and 5 is the right one — a wrong number
inside the paragraph written to fix a wrong number. And the new test carried a
dead binding, used zero times.

Measured on both surfaces after: **0 of 104** rail excerpts and **0 of 12**
drawer clips carry markup, median **1462 to 244** characters, the Sony Walkman
...
**Wave 6 — the room after it files.** `F-007`, `F-008`, `F-011`, closed
2026-08-11. Three rows, three commits, one surface.

Two of `F-007`'s fix direction changed under measurement, and the rows keep their
original text as always:

- **It scrolls the card, not the toggle.** The toggle sits below the drawer's
  plate, so aligning it to the top of the scroller cuts off the head of the very
  thing the reader asked to see.
- **The call belongs in the click handler, not inside `setOpen`.**
  `setOpen(false)` runs once at construction for all four drawers, and a future
  caller restoring an expanded drawer must not yank the page around.

`.drawer` also needed `scroll-margin-top: var(--tab-rise, 1.75rem)`, which no row
anticipated: the cut tab is positioned that far above the card's box, so a
top-aligned scroll slices it off the card it names.

**The brief left one question open and measuring answered it: closing needs no
scroll.** With the card open at top 28 and the scroller at 946, closing returns
the card to top 28 with the scroller at 323 — the browser's own scroll anchoring
compensates when content above the viewport shrinks, and a close-side scroll
would fight it. Not filed as a finding, because there is no defect.

**`F-011`'s "0px overlap" and a box measurement of 1.6px are both true.** The
clip-path makes the painted extent narrower than the box, and 1.6px at a sloped
edge scans as nothing. The row's instruction to adjust the cut in the same edit
was right and is now quantified: with the old percentages against a 27% taller
box the sides reached **2.77px** of inset at the face line where they used to
reach **0.59px**, flattening the visible taper by ~4.4px across the bottom edge.
Carrying the slope past the box restores it, re-measured at **0.62px**.

**The register's "cannot drift" claim on the grid held, and was checked rather
than trusted:** card tops are 351, 351, 1089, 1089 after the change, identical to
the measurement taken before it.

Two of the wave's own tests were wrong first and were caught by mutation, not by
review: the `z-index` assertion ran against the whole stylesheet, where
`.drawer { z-index: 0 }` satisfied it and a tab painted at 3 went straight
through; and the server-side assertion took a fixed 600-character window that
does not reach `args = call.args` fifteen lines above the increment, so it failed
against correct code.

**Wave 7 — said twice, meant once.** `F-018`, `F-014`, `F-016`, `F-017`, `F-019`,
`F-020`, closed 2026-08-11 in three commits.

Two of these rows render byte-identical pixels, and wave 3 passed on them for
that reason. The argument for doing them was `F-016`, where the same duplication
had already drifted far enough to be **visible**: the bible and the docket stack
in one column and share a left edge at x=348, but their first characters sat at
x=384 and x=376. Measured after: **0**.

**The thesis then appeared inside the wave's own evidence.** `F-017`'s four pairs
were not in one condition: the stamp and the url were value-identical, but the
other two had **already drifted by 0.05rem** — 0.85 against 0.9, 0.4 against 0.45
— with nothing anywhere arguing either number. The clip's values win, because the
drawer surface predates the check surface, so the rail's numbers are the copy.
**That moves the rail's two margins by 0.8px**, which is the one place in the
merge where a pixel changes, and it is recorded rather than presented as a no-op.

`F-018` was verified in the live DOM rather than inferred: every drawer stamp
renders `rgb(27, 33, 29)`, which is `--ink` exactly, and aniline appears on none
of them. The code was right and the row describing it was not.

**An existing test needed updating rather than working around**, and it is the
useful kind of finding: `test_scriptcheck.mjs` asserted the UNSOURCED stamp is
oxide by reading `scene.css`, and the rule now lives in `shell.css`. Its 2.5deg
rotation ceiling was checking `scene.css` alone and would have gone **quietly
green** the moment the stamp's `rotate(-2deg)` moved out of it — exactly what
this wave did. The ceiling now follows the rule across both files.

Card paddings went from three distinct values to two, and the test asserts that
as a **count**, so a third cannot quietly return.

**Wave 8 — the last four.** `F-015`, `F-022`, `F-023`, `F-024`, closed
2026-08-11. **Every finding in the register is now clean.**

Three of the four were one defect wearing three hats: something the reader did
left them with nowhere to stand. **Two of them were filed by this campaign's own
builds** — wave 3 found them while closing `F-006` and declined them as
drive-bys under the scope law, which is the whole argument for that law.

- **`F-023`** is `F-006`'s mechanism on the intake surface, and BOTH of
  `buildRoom`'s failure exits leaked it. `F-010`'s `role="alert"` does not
  substitute: an alert announces wherever focus sits and leaves a keyboard
  reader nowhere to go.
- **`F-022`** takes focus rather than a role, and the choice is mechanical.
  Both refusals are built WITH their text inside a subtree `draw()` inserts
  whole, so a role there is the fragile insert-with-text alert — and unlike the
  progress timeline's first entry, where that costs one announcement out of
  many, here it is every announcement there is. The move lives in `redraw`,
  which already clears `issueError` on every path that is not a fresh failure.
- **`F-024`** keeps the rebuild the code argues for and pays its cost back
  rather than overturning it.
- **`F-015`** is unrelated and was simply what was left. Both server messages
  declare the failure in their own first clause, verified in `star/server.py`,
  so the prefix framed nothing.

Ten mutations reintroduced, ten caught, and **three needed the test tightened
first**: one assertion was satisfied by the flag's own declaration, one by a
second `replaceChildren` elsewhere in the file, and one checked only
`renderAccountCard`'s body while the refusals are built in helpers it calls.
...
only row that seemed to explain it. The measurement says `F-002` does not
explain it, and turned up `F-025`, which does.

What the measurement changed, on the record:

- **`F-002`'s fix would make 44% of real cards worse, not better.** It proposes
  demoting `VERDICT_READING` from a sentence to a slug beside the stamp. But
  **four of the nine cards in the filed Gdansk check carry no note at all** —
  every one of them `confirmed` — because `verdicts.py:91` requires a note only
  for `unverifiable`. On those four, that sentence is the only prose on the
  card. Demote it and they become a stamp, a three-word slug, and 2,900
  characters of Wikipedia. The row names this hole as a precondition; the
  measurement says it is the main case, which makes `F-002` a **backend contract
  question before it is a copy fix**.
- **The card's proportions are the finding `F-002` was reaching for.** On the
  Sony Walkman card the answer is 21px of 1730 — **1.2%**. The gloss above it is
  2x the answer, the standing caveat 4x, and the citation block **68x**.
  Reordering prose cannot fix a 68:1 ratio; only `F-025` can.

Sequenced as two waves, deliberately, because they carry different risk.
`F-025` is self-contained and touches no contract. `F-002`'s real fix reaches
`star/verdicts.py`, and requiring a note for every verdict would push claims the
model left unannotated into `unverifiable` — trading empty cards for a worse
verdict distribution. That needs its own measurement against a real run before
it is written, not a decision made while building something else.

## The register

**F-001 to F-021 are the round-1 survivors.** F-022 and up were found during wave
builds — by the build, not by a review lens — and have faced no skeptic. Their
severity and visibility are the builder's estimate, and they are listed here so a
later round does not re-find them, not as audit output. Every one of them was
declined as a drive-by at the moment it was found: a wave ships only its
findings, and that rule is worth nothing if it bends when the fix is one line.
...
| F-020 | consent page | consistency | 2 | 1 | `consent.html:70-71`, `:8-43`; `consent.css:15-34`, `:31-34`, `:136-140`; `shell.css:219-223`; `server.py:1911` | `consent.html` links only `/tokens.css` and `/consent.css`. `.star-mark` is byte-identical in two files, carrying a `1.7rem` literal, and both files pre-authorize the collapse in comments addressed to "the orchestrator." The standalone-ness is deliberate but the stated reason is applicability, not isolation, and the CSP is `style-src 'self'` with everything same-origin. | **Ship the token, not the link.** `--mark-lg: 1.7rem` in `tokens.css` — already loaded by both documents — kills the duplication with zero new dependency. Linking `/shell.css` imports 36.5KB and three unmeasured globals onto the one screen where a reader hands something away: `a { color: var(--aniline) }` at 4.37:1 on manila, `:focus-visible` at 1.88:1 on manila, and a `body` flex layout `consent.css` does not override. Treat that as a separate, measured call. **Correction:** the focus-ring is not a true duplicate — shell.css sets only `outline-color` over a global shorthand that the consent page has no equivalent of. | clean |
| F-021 | marked scene page | a11y | 2 | 1 | `scene.css:250-266`; `scriptcheck.js:206-207`, `:546-553`; `server.py:1071-1094` | A scene up to 8000 chars sits in a `max-height: 32rem` scroller, and the zero-marks state is **designed for** — the server writes a dedicated cover note for "a scene of pure interior dialogue [that] asserts nothing about the world," and the rail ships bespoke copy for it. In that state the scroller holds no focusable child, and Safari still has no native scroller focus, so a keyboard-only Safari user cannot reach their own pasted pages. | `tabindex="0"` plus a role and accessible name on `.scene-page` — these must land **together** or the tab stop is unnamed. Prefer `role="region"` over `role="group"`: MDN's guidance is that `group` should not carry major perceivable sections, and region earns a landmark stop. `aria-label` on the current bare div is an ARIA authoring-conformance error (`generic` is name-prohibited) though all three engines do expose it. **Browser facts corrected:** Firefox has made scrollers tab stops since Firefox 4 (2011), so this is Safari-only, not two engines; Chrome's stable landing was 132, not 127. Accept the permanent tab stop deliberately — a short scene that never overflows still gets one. | clean |
| F-022 | your card, on a failed link or a failed issue | a11y | 3 | 1 | `account.js:286`, `:436`, `:570`, `:281-284`; `tests/js/test_token_retention.mjs` | Found during the F-010 build, and **F-010's own enumeration misses it.** `.account-refusal` renders a failed Google link (`:286`) and a failed token issue (`:436`). Both are failure-only: `linkMessage` is populated solely when `outcome.status === "failed"` (`:570`), and the comment at `:281-284` says so in its own words. Neither carries a role, so the two failures on the surface holding the credential are exactly as silent as the ones F-010 does name. F-010's headline is "every failure message in the app", which makes this a completeness defect in the finding, not only in the code. | **Not the same fix as F-010, which is why it is filed separately rather than folded in.** Both ship their text at construction inside a subtree `draw()` inserts wholesale, so a role on the constructed node is the fragile insert-with-text alert — and fragile on every occurrence here, not once per run as in F-012. `renderAccountCard` is a pure render function whose purity `test_token_retention.mjs` deliberately guards, so writing into a standing container means restructuring it. The deterministic alternative is F-006's shipped pattern: move focus to the refusal after `draw()`. Pick one before building; do not ship the role and call it covered. | clean |
| F-023 | intake, on a failed build request | a11y | 3 | 2 | `app.js:496`, `:553-554`; measured in Chromium 2026-08-11 | The mechanism F-006 diagnosed on the check surface, still live on the intake one. `$("build-btn").disabled = true` (`:496`) drops focus to `<body>`, the error is written at `:553`, and `:554` re-enables the button without giving focus back. Measured rather than reasoned: focus `build-btn` → disable → `activeElement` is `BODY` → re-enable → still `BODY`. A keyboard reader who tabs after a failed build restarts at the top of the document. | One line after the re-enable, matching the failure exit `scriptcheck.js` now ships: re-enable first, then `$("build-btn").focus()`, because `focus()` on a disabled element is a no-op. **Independent of F-010 in both directions** — a live region fires wherever focus happens to sit, so F-010's build is correct without this, and this is correct without F-010. Do not let the two be closed by one edit and one test. | clean |
| F-024 | check panel, opening a filed check | a11y | 2 | 1 | `scriptcheck.js:888`, `:896-900`, `:925-928`; observed in Chromium 2026-08-11 | Clicking a filed-check row leaves `activeElement` on `<body>`. `openFiledCheck` ends by rebuilding the filed row — `loadedFiledFor = null; openedCheck()` → `loadFiledChecks` → `els.filedList.replaceChildren()` (`:888`) — which destroys the button that was just clicked. The comment at `:925-928` documents the rebuild and defends it on single-source-of-truth grounds (one path decides `aria-current`); the focus it costs is not mentioned there. | Opening a filed check is not a paid request, which is why F-006 left it alone. Either patch `aria-current` in place instead of rebuilding — which the `:925-928` comment argues against, and that argument is sound — or re-focus the equivalent row after the rebuild by `sceneId`, which keeps the rebuild and its reasoning intact. Prefer the second. Note `mountResult`'s `moveFocus` is correctly `false` on this path: the fix is returning the reader to the row they pressed, not sending them to the result. | clean |
| F-025 | every excerpt, both evidence surfaces | visual + copy | 4 | 4 | `scriptcheck.js:21-31`, `:95-110`, `:298-306`; `clip.js:156-172`; `findings.py:135`; `13-check-annotated`; measured in Chromium 2026-08-11 | **The app's primary evidence surface renders raw scraped markdown as literal characters,** and no lens filed it — a grep of this register for "excerpt" returned nothing across all 67. Measured on the filed Gdansk room: **17 of 17** citation-rail excerpts carry markdown artifacts, and **10 of 12** drawer clips do. Across the 17: 73 pipe characters (table rows), 50 headings, 44 `**`, 30 `_italic_`, 17 `[n]`, 16 `[[ n ]]()`. Several excerpts do not begin with prose at all — one opens `\|Entry Gate No.`, one `# Lista uzbrojenia i wyposażenia...` followed by a table row, one `[](https://en.wikipedia.org/wiki/File:GDANSK,_Falowiec…)`, an empty image link rendered as brackets and a URL. Excerpt length runs 917-2573 characters, median **1462**, with **no cap anywhere in `web/`**. The result is that the citation block is **1421px of a 1730px card** — 82% — while the answer it supports is 21px, and every one of the nine cards runs 1100-2087px in a ~830px viewport, so no card can be read without scrolling. This is the "looks like a hackathon demo" reaction recorded mid-campaign, and it is a better explanation of it than F-002's verdict sentence. | **Strip markdown structure, then take prose, then cap — in that order.** A naive character cap or first-sentence cap is wrong here and would frequently cap a table cell, which is why the order matters: three of the sampled excerpts have non-prose as their opening. **Correct the contract first.** Both files' comments describe an HTML payload — `scriptcheck.js:21-31` cites `<strong>` highlighting and `&quot;` entities "verified against the stored Detroit-1929 room" — and the excerpts observed today are markdown. `clip.js`'s DOMPurify allowlist exists to preserve a `<strong>` present in **1 of 12** clips. Either the API's output changed or it varies by source; establish which before writing a stripper against the wrong format. The full source stays one click away, which is already how the citation works, so a cap hides nothing a reader cannot reach. | clean |
