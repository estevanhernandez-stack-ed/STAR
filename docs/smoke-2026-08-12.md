...
## Current as of `star-00065-v97` — read this before filming

This list was written on 2026-08-12 and stayed accurate through the 13th. Four
days of work landed after it. **The steps above still run**; what follows is
what now happens differently, and one step that no longer exists to be run.

### Step 5, the round trip, changed the most

- The confirm button **moves**. While a confirmation is pending it leaves the
  controls row and renders inside the report, last, under the count and the
  skipped rows. It comes home when there is nothing to confirm. Step 5 used to
  say "press it again" and left a reader hunting three lines up the page.
- **A fold-out sits above it.** "25 claims would change. Show exactly what" —
  open it and every claim names the note it takes, whether it is struck or
  un-struck, and **the words any note replaces**. That last one is the reason it
  exists: re-importing a stale copy silently overwrites what was typed since.
- **A file from a different sweep is refused**, and the refusal names the other
  sweep the way the picker names it — "24 scenes · 64 claims · 2026-08-13 11:41"
  — rather than by an id that appears on no screen.
- **The sweep picker labels to the minute.** Two sweeps of one draft on one day
  used to render as identical buttons.
- **Sweep exports carry the sweep id in the filename.** Three downloads on one
  day used to share a name and let the browser disambiguate with `(1)`.

### Step 9, the agent door, gained a tool and lost a gap

`import_notes` is the fifteenth tool. The annotation round trip is no longer
browser-only: export a sweep, mark it up, send it back through the door. Same
two-call arming, same refusals, same `_file_notes` behind both. **Step 9's note
that "no annotation import exists on this door" is obsolete.**

### The verdicts themselves changed, and this is what the video is about

- **The desk reads the year off the page.** Claims are judged against the years
  of the scenes that assert them, not the room's era. `<story_era>` is gone from
  the prompt entirely — it was cited in every wrong verdict for four rounds.
- **A dateless draft is told so** rather than guessed at: `unverifiable`, note
  "the draft states no year."
- **An imported room cannot launder a claim.** Its findings reach the verifier
  under a provenance banner, and a claim resting only on imported material
  cannot come back confirmed.

### Two operational changes with no visible surface

- The **daily cap persists** to `/service/daily_cap`. A deploy no longer returns
  a hundred rooms to the world. **The first build after a deploy creates that
  document** — worth checking once, and it is free.
- A build interrupted mid-flight now **keeps what it had researched** and gets
  its cap slot back.

### What has not changed and is still open

The citation defect. Verdicts are right and some receipts under them are not —
the `1959 Standard Vanguard Estate` is still sourced to a Ford Thames Pickup
listing. **Do not put a random confirmed row on screen and read its source
aloud** without checking it first. The anachronisms are reliable; a spot-checked
confirmation is fine; an unchecked one is a coin flip.
